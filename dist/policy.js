...
export function createHyperlinkPolicy(options = {}) {
    ...
    function normalize(input, context = {}) {
        ...
        if (probedScheme && DANGEROUS_SCHEMES.has(probedScheme))
            return reject('unsafe_scheme');
        if (CONTROL_RE.test(value) || value.includes('\\'))
            return reject('invalid_url');
        const phone = normalizePhone(value);
        if (phone)
            return accept({ input: value, href: phone, kind: 'phone' }, context, resolved);
        ...
        if (schemeMatch) {
            ...
            if (!value.slice(schemeMatch[0].length))
                return reject('invalid_url');
            if (!passesProtocol(protocol, value, context))
        ...
        if (value.startsWith('//')) {
            if (!resolved.allowProtocolRelative)
                return reject('invalid_url');
    ...
    function find(text, context = { source: 'autolink' }) {
        if (!text || text.length > resolved.maxLength * 4)
            return [];
        const matches = [];
        for (const match of linkify.match(text) ?? []) {
            if (match.schema === '//' && !resolved.allowProtocolRelative)
                continue;
            if (match.schema === 'mailto:' && !resolved.detect.emails)
                continue;
            if (match.schema !== 'mailto:' && !resolved.detect.urls)
                continue;
            const decision = normalize(match.raw, context);
            if (!decision.ok || (resolved.shouldAutoLink && !resolved.shouldAutoLink(decision, context)))
                continue;
        ...
        if (resolved.detect.phones) {
            for (const match of text.matchAll(PHONE_MATCH_RE)) {
                ...
                if (matches.some(candidate => from < candidate.to && candidate.from < to))
                    continue;
...
function createDetector(options) {
    ...
    for (const protocol of options.protocols.values()) {
        if (protocol.autolink === false)
            continue;
        detector.add(`${protocol.scheme}:`, { validate: CUSTOM_PROTOCOL_TAIL_RE });
    }
    return detector;
}
function accept(decision, context, options) {
    const accepted = { ok: true, ...decision };
    return options.validate && !options.validate(accepted, context) ? reject('rejected') : accepted;
}
function reject(reason) {
    return { ok: false, reason };
}
function hasSafeUnicodeHostnameCharacters(value) {
    return Array.from(value).every(character => /^[\x00-\x7f]$/u.test(character) ||
        /^[\p{L}\p{M}\u3002\uff0e\uff61]$/u.test(character));
}
function validateEmail(value) {
    if (value.length > 254 || value.split('@').length !== 2)
        return false;
    ...
    if (!local ||
        ...
        return false;
    if (/^[\x00-\x7f]+$/u.test(domain))
        return isPlausibleHostname(domain);
    try {
...
function exactMatch(linkify, value, schema) {
    const matches = linkify.match(value);
    return Boolean(matches?.length === 1 &&
        matches[0].index === 0 &&
        matches[0].lastIndex === value.length &&
        (!schema || matches[0].schema === schema));
}
function passesProtocol(protocol, href, context) {
