import { defineCommands, defineMarkSpec, Priority, union, withPriority, } from 'prosekit/core';
...
export { getActiveLink } from './range.js';
export function defineHyperlink(options = {}) {
    const resolved = resolveOptions(options);
    return union(defineResolvedHyperlinkSpec(resolved), defineResolvedHyperlinkBehavior(resolved));
}
function defineResolvedHyperlinkSpec(resolved) {
    ...
    return defineMarkSpec({
        ...
        toDOM(mark) {
            ...
            if (!decision.ok)
                return ['span', { 'data-hyperlink-invalid': '' }, 0];
...
function defineResolvedHyperlinkBehavior(resolved) {
    const { policy, defaultAttributes } = resolved;
    return union(defineCommands({
        addLink: (attrs, text) => addLinkCommand(policy, defaultAttributes, attrs, text),
        removeLink: () => removeLinkCommand(),
        toggleLink: (attrs, text) => toggleLinkCommand(policy, defaultAttributes, attrs, text),
        expandLink: () => expandLinkCommand(),
        updateLink: (update) => updateLinkCommand(policy, update),
    }), defineHyperlinkState({ shortcut: resolved.shortcut, policy }), withPriority(defineHyperlinkInteractions({
        policy,
        defaultAttributes,
...
function addLinkCommand(policy, defaults, input, text) {
    return (state, dispatch) => {
        ...
        if (!selectionAllowsMark(state, type))
            return false;
        if (!empty) {
...
function toggleLinkCommand(policy, defaults, attrs, text) {
    return (state, dispatch, view) => {
        ...
        const hasLink = state.selection.empty
            ...
            : state.doc.rangeHasMark(state.selection.from, state.selection.to, type);
        return (hasLink ? removeLinkCommand() : addLinkCommand(policy, defaults, attrs, text))(state, dispatch, view);
    };
}
function expandLinkCommand() {
    return (state, dispatch) => {
...
function updateLinkCommand(policy, update) {
    return (state, dispatch) => {
        const range = getActiveLink(state);
        const type = state.schema.marks.link;
        if (!range || !type)
        ...
        if (update.text !== undefined) {
            if (!update.text || !isSimpleTextRange(state.doc, range.from, range.to))
                return false;
