import { defineClickHandler, defineKeyDownHandler, definePasteHandler, definePlugin, defineTextInputHandler, union, } from 'prosekit/core';
import { defineEnterRule } from 'prosekit/extensions/enter-rule';
import { defineInputRule } from 'prosekit/extensions/input-rule';
import { InputRule } from 'prosekit/pm/inputrules';
import { Fragment, Mark, Slice, } from 'prosekit/pm/model';
import { Plugin, PluginKey, TextSelection } from 'prosekit/pm/state';
import { getActiveLink, selectionAllowsMark } from './range.js';
import { hyperlinkPluginKey, normalizeHyperlinkHref } from './state.js';
const AUTOLINK_INPUT_RE = /([\s\S]{1,500})(\s)$/u;
const AUTOLINK_ENTER_RE = /[\s\S]{1,200}$/u;
const TRAILING_PUNCTUATION_RE = /^[.,;:!?…)}\]”’'"»]*$/u;
const MARKDOWN_LINK_RE = /(^|[^!])(\[([^\]\n]+)\]\(([^()\s]+)\))$/u;
const automaticLinkPluginKey = new PluginKey('prosekit-hyperlink-automatic');
export function defineHyperlinkInteractions(options) {
    const extensions = [definePasteHandler(createPasteHandler(options))];
    if (options.openOnClick) {
        extensions.push(defineClickHandler(createLinkClickHandler(options)));
    }
    if (options.autolink) {
        extensions.push(definePlugin(createAutomaticLinkPlugin()), defineTextInputHandler((view, from, to, inserted) => {
            ...
            return true;
        }), defineInputRule(createAutolinkInputRule(options)), defineEnterRule({
            regex: AUTOLINK_ENTER_RE,
            stop: false,
            handler: ({ state, from, match }) => addTrailingDetectedMark(state, state.tr, from, match[0], options),
        }));
    }
...
function createLinkClickHandler(options) {
    return (view, position, event) => {
        ...
        if (options.selectOnClick) {
            transaction = transaction.setSelection(TextSelection.create(transaction.doc, active.from, active.to));
        }
        const next = {
            mode: 'preview',
...
function createAutolinkInputRule(options) {
    return new InputRule(AUTOLINK_INPUT_RE, (state, match, start, end) => {
...
function createMarkdownLinkInputRule(options) {
    return new InputRule(MARKDOWN_LINK_RE, (state, match, start, end) => {
        ...
        if (!mark || !parent.type.allowsMarkType(type))
            return null;
...
function addTrailingDetectedMark(state, transaction, textStart, text, options) {
    const type = state.schema.marks.link;
    ...
    if (from >= to || state.doc.rangeHasMark(from, to, type))
        return null;
    if (rangeHasCodeMark(state.doc, from, to))
        return null;
    const mark = createValidatedMark(type, {
        ...options.defaultAttributes,
        href: candidate.href,
    });
    if (!mark || !state.doc.resolve(from).parent.type.allowsMarkType(type))
        return null;
    return transaction
        .addMark(from, to, mark)
        .setMeta(automaticLinkPluginKey, { from, to, href: candidate.href });
}
function createAutomaticLinkPlugin() {
    return new Plugin({
        ...
        state: {
            ...
            apply(transaction, previous, _oldState, newState) {
                ...
                const ranges = previous
                    ...
                    .filter((range) => Boolean(range && automaticRangeStillLinked(newState, type, range)));
                const added = transaction.getMeta(automaticLinkPluginKey);
                if (added && automaticRangeStillLinked(newState, type, added))
                    ranges.push(added);
                return ranges;
            },
        },
...
function createHyperlinkKeyDownHandler(options) {
    return (view, event) => {
        ...
        if (options.autolink && (event.key === 'Backspace' || event.key === 'Delete')) {
            ...
            if (deletion) {
                ...
                view.dispatch(reconcileAutomaticRange(view.state, transaction, deletion.range, options));
                return true;
            }
        }
...
function reconcileAutomaticRange(state, transaction, range, options) {
...
function mapAutomaticRange(range, transaction) {
...
function findAutomaticEditRange(state, from, to) {
    return (automaticLinkPluginKey.getState(state) ?? []).find(range => from === to
        ? from > range.from && from < range.to
        : from >= range.from && to <= range.to) ?? null;
}
function automaticDeletion(state, key) {
    const { selection } = state;
    const ranges = automaticLinkPluginKey.getState(state) ?? [];
    if (!selection.empty) {
        const range = ranges.find(candidate => selection.from >= candidate.from && selection.to <= candidate.to);
        return range ? { from: selection.from, to: selection.to, range } : null;
    }
    const position = selection.from;
    const range = ranges.find(candidate => key === 'Backspace'
...
function exitLinkAtEdge(view, key) {
...
function automaticRangeStillLinked(state, type, range) {
    ...
    state.doc.nodesBetween(range.from, range.to, node => {
        ...
        if (!mark || mark.attrs.href !== range.href)
            valid = false;
        return valid;
    });
    return sawText && valid;
}
function findTrailingCandidate(text, options) {
...
function createPasteHandler(options) {
    return (view, event, slice) => {
        ...
        if (Array.from(clipboard?.types ?? []).includes('application/x-prosemirror') ||
...
function transformFragment(fragment, type, options, allowTextMarks) {
    let changed = false;
    const children = [];
...
function transformNode(node, type, options, allowTextMarks) {
...
function linkifyTextNode(node, type, options) {
    ...
    for (const match of matches) {
        if (match.from < offset || match.from >= match.to)
            continue;
        if (offset < match.from)
            nodes.push(node.type.schema.text(text.slice(offset, match.from), node.marks));
        const mark = createValidatedMark(type, {
            ...options.defaultAttributes,
            href: match.href,
        });
    ...
    if (offset < text.length)
        nodes.push(node.type.schema.text(text.slice(offset), node.marks));
    return nodes;
}
function uniformMarks(doc, from, to, excluded) {
    let expected;
    let uniform = true;
    doc.nodesBetween(from, to, node => {
        ...
        if (!expected)
            expected = marks;
        else if (!Mark.sameSet(expected, marks))
            uniform = false;
        return uniform;
    });
    return uniform ? (expected ?? []) : null;
}
function createValidatedMark(type, attrs) {
    try {
...
function rangeHasCodeMark(doc, from, to) {
    let found = false;
    doc.nodesBetween(from, to, node => {
...
function isMarkdownDestination(text, offset) {
