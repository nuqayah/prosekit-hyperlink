export function getActiveLink(state, position) {
    const type = state.schema.marks.link;
    const usesSelection = position === undefined;
    const resolvedPosition = position ?? state.selection.from;
    if (!type || resolvedPosition < 0 || resolvedPosition > state.doc.content.size)
        return null;
    ...
    for (let index = child.index - 1; index >= 0; index--) {
        ...
        if (!sibling.marks.some(candidate => candidate.eq(mark)))
            break;
...
export function selectionAllowsMark(state, type) {
    const { selection } = state;
    if (selection.empty)
        return selection.$from.parent.type.allowsMarkType(type);
    let sawInline = false;
    let allows = true;
...
export function isSimpleTextRange(doc, from, to) {
