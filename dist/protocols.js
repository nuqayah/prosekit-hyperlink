...
export const applicationProtocols = APPLICATION_PROTOCOL_ENTRIES.map(([scheme, type, title, category]) => ({
    scheme,
    type,
    title,
    category,
}));
export const communicationProtocols = applicationProtocols.filter(protocol => protocol.category === 'communication' || protocol.category === 'social');
export const developerProtocols = applicationProtocols.filter(protocol => protocol.category === 'development');
const schemeInfo = new Map([
    ['mailto', { type: 'email', title: 'Email', category: 'communication' }],
    ['tel', { type: 'phone', title: 'Phone', category: 'communication' }],
    ...applicationProtocols.map(({ scheme, type, title, category }) => [scheme, { type, title, category }]),
]);
const domainInfo = DOMAIN_ENTRIES.map(([domain, type, title, category]) => [domain, { type, title, category }]);
/** Classify a configured app scheme or a well-known web domain. */
export function getApplicationLinkInfo(href) {
